...
This InSpec Profile was created to facilitate testing and auditing of `Kubernetes`
...
The Kubernetes STIG Profile uses the [InSpec](https://github.com/inspec/inspec)
...
Requirements associated with the Kubernetes STIG are derived from the
...
The Kubernetes STIG profile checks were developed to provide technical implementation
...
```yaml
  ...
  # used by the Kubernetes STIG profile.
...
```bash
# Using `ssh` transport
bundle exec [inspec or cinc-auditor] exec https://github.com/mitre/kubernetes-stig-baseline/archive/main.tar.gz --input-file=<your_inputs_file.yml> -t ssh://<hostname>:<port> --sudo --reporter=cli json:<your_results_file.json>

# Using `winrm` transport
bundle exec [inspec or cinc-auditor] exec https://github.com/mitre/kubernetes-stig-baseline/archive/master.tar.gz --target winrm://<hostip> --user '<admin-account>' --password=<password> --input-file=<path_to_your_inputs_file/name_of_your_inputs_file.yml> --reporter=cli json:<path_to_your_output_file/name_of_your_output_file.json>
```
...
mkdir profiles
cd profiles
git clone https://github.com/mitre/kubernetes-stig-baseline.git
bundle exec [inspec or cinc-auditor] archive kubernetes-stig-baseline
...
```bash
cd kubernetes-stig-baseline
git pull
cd ..
bundle exec [inspec or cinc-auditor] archive kubernetes-stig-baseline --overwrite
